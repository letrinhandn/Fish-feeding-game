...
def load_ascii_art(filename):
    ...
    path = os.path.join('assets', 'art', filename)
    try:
...
def safe_addstr(window, y, x, string, color_pair=0):
    """Safely add a string to the window, avoiding errors if it doesn't fit."""
    height, width = window.getmaxyx()
    # Check if the starting position is valid
    if y < 0 or y >= height or x < 0 or x >= width:
        return
    
    # Truncate the string to fit within the window
    max_length = width - x
    if max_length <= 0:
        return
    
    string_to_draw = string[:max_length]
    
    try:
        window.addstr(y, x, string_to_draw, color_pair)
    except curses.error:
        # Ignore curses errors from drawing outside of window
        pass
...
def main(stdscr):
    ...
    # Load music files
    music_files = [
        os.path.join('assets', 'music', file) for file in os.listdir(os.path.join('assets', 'music'))
    ...
    while not game_over:
        ...
        safe_addstr(stdscr, 1, 2, feeding_instructions, curses.color_pair(5))
        ...
        safe_addstr(stdscr, 1, width - len(music_controls) - 2, music_controls, curses.color_pair(5))
        
        # Draw exit instruction at a safe position below the music controls
        exit_text = "Press 'q' to exit"
        safe_addstr(stdscr, 3, width - len(exit_text) - 2, exit_text, curses.color_pair(5))
        ...
        for crab in crabs:
            draw_ascii_art(stdscr, crab.get_display_art(), int(crab.y), int(crab.x), 1)
        ...
        if current_music:
            ...
            safe_addstr(stdscr, 0, width - len(music_text) - 2, music_text, curses.color_pair(5))
